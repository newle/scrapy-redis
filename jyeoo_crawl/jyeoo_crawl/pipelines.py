...
def getSQL(url):
    s = "(type, label_txt, label_html, question_txt, question_html, ans_txt, ans_html, parse_txt, parse_html, comments_html, question_url) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);"
...
class JyeooCrawlPipeline(object):
    ...
    def process_item(self, item, spider):
        if 'question_html' in item and ('ans_html' in item or 'parse_html' in item):
            pass
        else:
        ...
        label_html = item['label_html'] if 'label_html' in item else ""
        type_html = item['type_html'] if 'type_html' in item else ""
        ...
        type_txt = html_to_text(type_html)
        ...
        print "type_html = " + type_html.encode('gb18030')

        #updatecur.execute(addtoraw, (url, question_html, "good"))
        #updateconn.commit()
        self.updatecur.execute(getSQL(item['question_url']), (type_txt, label_txt, label_html, question_txt, question_html, ans_txt, ans_html, parse_txt, parse_html, comments_html, item['question_url']))
